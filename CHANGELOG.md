...
- **feat(build)**：悬空引用检查——开启 `shipAssets` 后，构建自动验证产物文本里的包内路径引用指向的文件真的在包里；未命中打印「文件:行:引用」并构建失败。不开启时此检查不运行；自行组装的工程可继续直接调用同一函数（`scanDanglingRefs`）。
- **feat(cli)**：`skillnomad init <目录>`——从内置模板生成可直接构建的新项目骨架；目标目录非空即拒绝，不覆盖已有文件。模板随本版本首次进入发布包。
- **feat(cli)**：Node 低于 22.18 时 CLI 启动即提示并退出；包声明 `engines.node >= 22.18`（CI 覆盖 22.18 与 24）。
- **chore(demos)**：示例改为独立二级目录，保留最小两步 demo；demo 与模板不再依赖 tsx（Node 原生类型剥离）。
- **test**：新增双构建重放门（技能产物逐字节一致＋清单哈希链自洽）。
- **docs(site)**：「从模板开始」改用 `init`；发布布局页补随包搬运与悬空引用检查说明。
- 升级影响：无破坏性变更，API 与产物结构不变（新能力均需显式开启）。新增 `engines` 声明：npm 安装会提示引擎不兼容，`skillnomad` CLI 启动即报错并说明版本要求（此前要到构建失败才暴露原因）。

## v0.2.3（对外文本与注释清理 · 包元数据补全）

- **docs(site)**：站点文案收束——边界句改中立表述；去掉自造词与内部话术；承诺句改为与构建期对齐报告一致的可核验表述；文档指引改公开可达链接。
- **docs**：README 指引与措辞修正——默认配置说明改命令视角，类型/校验/构建路径改包内实际路径，补裸跑所需 Node 版本前提；CONTRIBUTING 删除指向不存在声明的句子；协作说明只留职责与约束。
- **docs(changelog)**：历史条目去掉内部优先级与里程碑编号、内部评审用语。
- **chore(comments)**：源码与测试注释清退内部编号与过程话术，保留职责与约束本身。
- **fix(cli)**：提示文案去掉内部代号；修正一处指向已退役入口的提示。
- **chore(pkg)**：description 去掉与包名重复的前缀；移除无对应实现的厂商关键词；补 homepage。
- 回归：typecheck／lint 通过；全量测试 115/115。
- 升级影响：无破坏性变更；API 与产物结构不变。
...
- **docs(api)**：公开面全量 TSDoc 分类（`@category` 五组）——文档站 TypeDoc 改 `src/index.ts` 单入口，51 个导出符号全部有生成页。
...
- 升级影响：旧三包导入改从主包导入；产物路径与调度相关产物需按新布局重建。
...
## v0.1.3（decision 示例分隔符）

- **feat(types)**：`SourceDecisionSummary` 加 `isExample?: boolean`（语义单真相源：为 true 时全块为历史运行示例值，非本次运行时填充；缺席即事实）——few-shot 示例分隔符的机器可读边界。
...
- **breaking(types)**：清退 `SourceRef.ref` 概念引用声明形态（删字段与注释块），`SourceRef.path` 转必填——该形态自 beta.9 发布起零真实用例：两次真实转化（官方示例 scenario-pipeline 与另一移植案例）均自发选择用户侧 `refOf` helper 达成同一不变量「步骤层零路径字面量」，原定验证路径被真实转化绕开而非采用。路径解析归用户侧，框架不承载概念引用形态——「不做领域模型抽象」的边界收得更紧。
...
- 端到端：官方示例 scenario-pipeline 构建产物 `## 增量复用`×4、`## 插件加载`×4、`## 依赖`×11 全命中（`## 降级协议`零命中符合预期：11 步零 `.degrade()`）。
- 升级影响：带正文步骤的产物新增缺失章节（补承诺，非新功能）；无正文步骤产物逐字不变。

## v0.1.0（首个稳定版 · 可信度里程碑）

> **定位**：semver 口径：0.x 的 minor 即可包含破坏性变更；本版承诺仅「默认安装即可用」，**契约冻结发生在 1.0**（`0.2.0` 起仍可破坏性变更）。

**本版做了两件事**：

- **安装面**：四包 + release-manifest 统一 0.1.0；`latest` dist-tag 自本版起首次指向正式版（此前停留 beta.1，默认安装落后 8 个版本）
- **契约对账**：`contract.md` 12 条公开承诺逐条复核通过（框架 45 测试 case + `skillnomad-validate` 独立入口实测）




**相对 v0.1.0-beta.9 的代码差异**：
...
## v0.1.0-beta.9（概念引用承载候选）

- **概念引用形态（产物路径投射的框架侧）**：
  - `SourceRef.path` 改为可选 + 新增 `ref?`（概念引用声明形态）。
  - 边界：框架**只承载形态，不做领域模型抽象**——`ref` 需由领域侧 resolver 解析为 `path` 后传入；`sourceRef`/map over 遇未解析 `ref` 抛错并提示。
  - 官方示例 scenario-pipeline 采用实体常量模式（`domain/entities.ts` + `refOf`），步骤层路径字面量清零——框架渲染/校验零逻辑改动。
...
- 配套官方示例 scenario-pipeline：18 处 `skillnomad-types` import 收敛 + package.json 只声明 skillnomad；第二用例（移植案例）同步收敛。
- 验证：tsc + 16 测试全绿；官方示例 scenario-pipeline 44 测试全绿、产物无新增差异。

## v0.1.0-beta.7（Step 2 候选）

- **模块引用一致性校验**：
  - `SourceContract` 加 `scope: 'skill' | 'step'`（归属层）+ `step?`（step 级归属步骤）。
  - `SourceRefRole` 加 `'method'`。
...
- **dependsOn 单值化**：`dependsOn: string[]` → `dependsOn?: string`；builder `.dependsOn(id: string)`；`next` 加 `@deprecated` 注解。
- **contractRefs 删除**：`SourceInstruction.contractRefs` 删除；`SourceRef` 新增 `as?: 'contract'|'schema'|'rule'|'reference'` 角色标签。
