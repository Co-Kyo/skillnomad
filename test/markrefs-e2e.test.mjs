...
// P1 E2E：真实 CLI 跑最小 skill 源的 markrefs 校验——缺引用必须让构建失败（"拦不住＝没做成"）。
// 夹具为纯 ESM（不经 tsx），模式用 env 切换：ok / missing-target / missing-key / dup-path / drift。

const here = dirname(fileURLToPath(import.meta.url));
const pkgRoot = join(here, '..');
const appDir = join(here, 'fixtures', 'markrefs-app');
...
function build(mode, strict = false) {
    const result = spawnSync(process.execPath, [cli, 'build', 'skillnomad.config.mjs'], {
        cwd: appDir,
        encoding: 'utf8',
        env: { ...process.env, MARKREFS_MODE: mode, MARKREFS_STRICT: strict ? '1' : '' },
    });
