...
| skillnomad（本仓） | 装配与产物层 | 组合、解析、验证、编译、渲染；链序、编号、区间、发布路径全部由声明派生 | 不再拥有业务能力（冻结新增：新能力先回答"为什么不能是内容包"）；零调度（并行/分批只在步骤内 flow，不在声明层） |
...
工具依赖一律固定版本号（整组同版，发版联动须同步全部 pin）。
