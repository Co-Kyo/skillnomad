...
校验由 `validateStepChain()` 实现，在 `skillnomad build` 与 `skillnomad validate` 两处都会执行；
...
以上推导共用同一个底座 `resolveChain()`：走一遍链，返回有序 id 列表（下标即序号）。集中一处的原因是——每个派生函数各写一遍遍历，就会各漂移一遍。
