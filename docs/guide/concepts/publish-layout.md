...
- `scanSourcePaths`（构建期自动）：产物里出现源码形态路径（`src/…`）即红——散文也不行，产物文本只说发布形态。
- `scanDanglingRefs`（悬空引用检查）：产物文本里引用了 `steps/…`／`references/…` 等包内路径，但包里解析不到即红（含历史布局遗留名——写了就是悬空）。模板（含 `{}`）与通配（含 `*`）不参与。开启 `shipAssets` 后构建自动执行它（扫描范围＝渲染＋搬运的全部 markdown）；不开启的工程可继续对任意文件集直接调用同一函数。
...
::: tip 写作提醒
指针与正文同文件但相距可能上百行——域绑定的具体动作（校验哪个字段、编织哪个关系）请写在任务正文里，模块附录只承载通用方法论；否则执行侧按正文行事时会查不到材料。
:::
